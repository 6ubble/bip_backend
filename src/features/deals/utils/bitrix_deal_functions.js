...
const createDealWithFiles = async (dealFields, files = []) => {
    ...
    const originalTitle = dealFields.TITLE;
    
    // Создаем сделку сначала без TITLE, чтобы получить автогенерированный шаблон
    const dealData = {
        fields: { 
            ...dealFields,
            UF_CRM_1756703320: 1 
    ...
    // Убираем TITLE из первоначального запроса
    delete dealData.fields.TITLE;

    const dealId = await bitrixRequest('crm.deal.add', {}, 'POST', dealData);
    if (!dealId) throw new Error('Failed to create deal');

    // Получаем сделку с автогенерированным названием
    const createdDeal = await bitrixRequest('crm.deal.get', { ID: dealId });
    const autoGeneratedTitle = createdDeal.TITLE || '';
    
    // Объединяем наш заголовок с шаблоном Bitrix
    const finalTitle = `${originalTitle}${autoGeneratedTitle}`;
    
    // Обновляем с финальным заголовком
    try {
    ...
    // Остальной код с файлами остается без изменений...
    if (files && files.length > 0) {
        ...
        if (uploadedFileIds.length > 0) {
            try {
                ...
                console.log('Files attached to activity only');
            }
        }
    }
